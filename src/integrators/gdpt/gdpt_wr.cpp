...
GDPTWorkResult::GDPTWorkResult(const ReconstructionFilter* rfilter, Vector2i blockSize, int extraBorder) {
...
GDPTWorkResult::~GDPTWorkResult() {
}

void GDPTWorkResult::put(const GDPTWorkResult* workResult) {
	for (size_t i = 0; i < m_block.size(); ++i) {
		m_block[i]->put(workResult->m_block[i].get());
	}
}

void GDPTWorkResult::clear() {
	for (size_t i = 0; i < m_block.size(); ++i) {
		m_block[i]->clear();
	}
}
...
void GDPTWorkResult::load(Stream* stream) {
	for (size_t i = 0; i < m_block.size(); ++i) {
		m_block[i]->load(stream);
	}
}

void GDPTWorkResult::save(Stream* stream) const {
	for (size_t i = 0; i < m_block.size(); ++i) {
		m_block[i]->save(stream);
	}
}

std::string GDPTWorkResult::toString() const {
	return m_block[0]->toString();	//todo
}


MTS_IMPLEMENT_CLASS(GDPTWorkResult, false, WorkResult)
MTS_NAMESPACE_END